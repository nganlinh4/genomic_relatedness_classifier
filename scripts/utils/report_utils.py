...
def build_markdown_en(consolidated, reports_dir):
    ...
    lines = []
    # Insert CSS style block for PDF rendering (must be at top of markdown)
    lines.append('<style>')
    lines.append('    /* 1. Set the PDF page margins to be smaller (Top Right Bottom Left) */')
    lines.append('    @page {')
    lines.append('        margin: 15mm 10mm 15mm 10mm;')
    lines.append('        size: A4 portrait;')
    lines.append('    }')
    lines.append('')
    lines.append('    /* 2. Override the default limit that centers text with big white space */')
    lines.append('    .markdown-body {')
    lines.append('        max-width: 100% !important;')
    lines.append('        min-width: 100% !important;')
    lines.append('        width: 100% !important;')
    lines.append('        margin: 0 !important;')
    lines.append('        padding: 0 !important;')
    lines.append('    }')
    lines.append('')
    lines.append('    /* 3. Ensure tables use full width and fonts are small enough to fit */')
    lines.append('    table {')
    lines.append('        width: 100% !important;')
    lines.append('        display: table !important;')
    lines.append('        font-size: 10px !important; /* Adjust if still too big */')
    lines.append('    }')
    lines.append('    ')
    lines.append('    /* Optional: tightening cell padding */')
    lines.append('    th, td {')
    lines.append('        padding: 4px !important;')
    lines.append('    }')
    lines.append('</style>')
    lines.append('')
